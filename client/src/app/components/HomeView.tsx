...
const FALLBACK_PHOTOS = [
  ...
  },
  {
    ...
    alt: 'Пустой бассейн с шезлонгами',
  },
  {
  ...
  },
  {
    ...
    alt: 'Открытый бассейн в тропиках',
  },
];
    ...
    <div>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', color: '#FBF6EB', padding: '96px clamp(20px,6vw,80px) 0' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: 40, alignItems: 'center', maxWidth: 1280, margin: '0 auto', paddingBottom: 70 }} className="hero-inner-grid">
            <div>
              <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 19.5, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#E3C77E', display: 'flex', alignItems: 'center', gap: 10, marginBottom: 30 }}>
                <span style={{ width: 26, height: 1, background: '#E3C77E', display: 'inline-block' }} />
                Бассейн Хюга · Отдых на набережной
              </div>
              <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(40px,5.2vw,68px)', lineHeight: 1.04, fontWeight: 700, maxWidth: 600 }}>
                Забронируйте тапчан заранее.{' '}
                <em style={{ fontStyle: 'normal', color: '#7FC2CC' }}>Обо всём остальном позаботимся мы.</em>
              </h1>
              <p style={{ marginTop: 22, fontSize: 17, lineHeight: 1.65, color: '#DCEEEF', maxWidth: 460, fontWeight: 400 }}>
                Забронируйте тапчан у бассейна онлайн, заказывайте блюда прямо со своего места или отсканируйте QR-код на тапчане, чтобы мгновенно открыть меню.
              </p>
              <div style={{ display: 'flex', gap: 14, marginTop: 34, flexWrap: 'wrap' }}>
                <button onClick={() => onNavigate('booking')} style={{ padding: '15px 26px', borderRadius: 100, fontWeight: 700, fontSize: 14.5, display: 'inline-flex', alignItems: 'center', gap: 8, background: '#BD5B38', color: '#fff', border: 'none', cursor: 'pointer', fontFamily: "'Manrope', sans-serif" }}>
                  Бронирование тапчана →
                </button>
              </div>
              <div style={{ display: 'flex', gap: 28, marginTop: 48, flexWrap: 'wrap' }}>
                {[{ val: '23', label: 'Тапчана' }, { val: '10:00–22:00', label: 'Время работы' }, { val: '~25 макс', label: 'Подача заказа' }].map(s => (
                  <div key={s.label}>
      ...
      <section style={{ padding: '88px clamp(20px,6vw,80px)', maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ marginBottom: 44 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16 }}>
            <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(28px,3.4vw,42px)', color: '#182422', fontWeight: 600 }}>
              Добро пожаловать в Hyuga Swimming Pool
            </h2>
            <p style={{ color: '#4B5C58', maxWidth: 380, fontSize: 15, lineHeight: 1.6 }}>
              Современная инфраструктура, чистый воздух и высокий уровень сервиса делают Hyuga Swimming Pool идеальным местом для отдыха.
            </p>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr', gridTemplateRows: '280px 240px', gap: 14, marginBottom: 14 }} className="gallery-grid">
          <div onClick={() => openLightbox(0)} style={{ gridColumn: '1', gridRow: '1 / 3', borderRadius: 20, overflow: 'hidden', position: 'relative', cursor: 'pointer' }}>
            <img src={galleryPhotos[0]?.url} alt={galleryPhotos[0]?.alt} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transition: 'transform 0.4s ease' }} />
            ...
            <div style={{ position: 'absolute', bottom: 18, left: 18, color: '#FBF6EB' }}>
              <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10.5, letterSpacing: '.12em', textTransform: 'uppercase', color: '#E3C77E' }}>Главный бассейн</div>
            </div>
          </div>
          <div onClick={() => openLightbox(1)} style={{ gridColumn: '2', gridRow: '1', borderRadius: 20, overflow: 'hidden', cursor: 'pointer' }}>
            <img src={galleryPhotos[1]?.url ?? galleryPhotos[0]?.url} alt={galleryPhotos[1]?.alt} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transition: 'transform 0.4s ease' }} />
          </div>
          <div onClick={() => openLightbox(2)} style={{ gridColumn: '3', gridRow: '1', borderRadius: 20, overflow: 'hidden', position: 'relative', cursor: 'pointer' }}>
            <img src={galleryPhotos[2]?.url ?? galleryPhotos[0]?.url} alt={galleryPhotos[2]?.alt} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transition: 'transform 0.4s ease' }} />
          ...
          <div onClick={() => openLightbox(3)} style={{ gridColumn: '2 / 4', gridRow: '2', borderRadius: 20, overflow: 'hidden', position: 'relative', cursor: 'pointer' }}>
            <img src={galleryPhotos[3]?.url ?? galleryPhotos[0]?.url} alt={galleryPhotos[3]?.alt} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transition: 'transform 0.4s ease' }} />
            ...
            <div style={{ position: 'absolute', left: 24, top: '50%', transform: 'translateY(-50%)', color: '#FBF6EB' }}>
              <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 24, fontWeight: 600, marginBottom: 4 }}>Зона отдыха</div>
              <div style={{ fontSize: 13, color: '#DCEEEF' }}>Окружена пышной растительностью</div>
            </div>
          </div>
        </div>

        <div style={{ borderRadius: 20, overflow: 'hidden', border: '1px solid rgba(14,58,57,0.12)', position: 'relative' }}>
          <div style={{ position: 'absolute', top: 18, right: 18, zIndex: 10, background: 'rgba(14,58,57,0.88)', backdropFilter: 'blur(10px)', borderRadius: 12, padding: '12px 18px', color: '#FBF6EB' }}>
            <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, letterSpacing: '.12em', textTransform: 'uppercase', color: '#7FC2CC', marginBottom: 4 }}>Наш адрес</div>
            <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 18, fontWeight: 600 }}>Юкори Чирчикский район, Ташкент</div>
            <div style={{ fontSize: 12.5, color: '#DCEEEF', marginTop: 2 }}>+998 95 801 10 60</div>
            <a
              href="https://yandex.uz/maps/10335/tashkent/?ll=69.444014%2C41.262841&mode=poi&poi%5Bpoint%5D=69.441563%2C41.263949&poi%5Buri%5D=ymapsbm1%3A%2F%2Forg%3Foid%3D8496875622&z=17.8"
              target="_blank"
          ...
          <iframe
            title="Hyuga Swimming Pool — Адрес"
            src="https://maps.google.com/maps?q=Hyuga,+Tashkent,+Uzbekistan&output=embed&z=14"
            width="100%"
            height="380"
